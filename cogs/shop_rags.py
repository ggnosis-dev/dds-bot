...
class RagsShopView(discord.ui.LayoutView):
	...
	def _build_shop_layout(self) -> None:
		...
		container.add_item(ui.TextDisplay(f"### Rag's Shop"))
	...
	def _purchase_callback(self, item_id: str):
		...
		async def callback(interaction: discord.Interaction) -> None:
			...
			if not check:
				await interaction.response.send_message(f"You don't have enough gems to purchase a **{item['display_name']}**.", ephemeral = True)
				return
