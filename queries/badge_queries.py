...
async def get_demon_badge_id(demon_id: int) -> int | None:
...
async def set_badge_on_player(player_id: int, badge_id: int) -> bool:
...
async def get_all_demon_badges(player_id: int) -> list[BadgeData]:
	"""Get the amount of magnetite a player has."""
	rows = query_all(
		"""
